 ...
 */
export const requestRestock = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  // Verificar que el beneficio pertenece al comercio
  ...
  // Registrar en AdminLog para auditoría
  await prisma.adminLog.create({
    data: {
      adminId: merchantId,
      action: 'RESTOCK_REQUESTED',
      targetId: benefit.id,
      description: `Reabastecimiento de ${amount} unidades para "${benefit.title}"`,
      metadata: {
        benefitId: benefit.id,
        benefitTitle: benefit.title,
        amountRequested: parseInt(amount),
        previousStock: benefit.stock,
        newStock: updatedBenefit.stock,
        notes: notes || null,
  ...
  successResponse(
    res,
    {
      benefit: {
        ...
        title: updatedBenefit.title,
        previousStock: benefit.stock,
        newStock: updatedBenefit.stock,
        amountAdded: parseInt(amount),
      },
    },
